...
-- Create Main Window
local window = library:AddWindow("Legends HUB", {
	main_color = Color3.fromRGB(34, 129, 231),
	local min_size = Vector2.new(400, 500),
	can_resize = true,
})
...
mainTab:AddLabel("ZeusLGD Script")
...
local farmGymsFolder = mainTab:AddFolder("Machines")
...
rebirthsFolder:AddTextBox("Rebirth Target", function(text)
	...
	if newValue and newValue > 0 then
		...
		game:GetService("StarterGui"):SetCore("SendNotification", {
			...
			Text = "Novo Objetivo: " .. tostring(targetRebirthValue) .. " renascimentos",
			Duration = 0
		})
	else
		game:GetService("StarterGui"):SetCore("SendNotification", {
			Title = "Entrada Inválida",
			Text = "Por favor coloque um número válido maior que 0",
			Duration = 0
		})
	end
end)
...
end, "Renacimiento automático em busca do objetivo")
...
end, "Renascimento continuo sem parar")
...
end, "Teletransporte continuo ao King Muscle")
...
end, "Desbloquea a gamepass de AutoLift gratis")
...
end, "Levanta pesos automáticamente")
...
end, "Faz flexões automáticamente")
...
end, "Faz paradas de mão automáticamente")
...
end, "Faz abdominais automáticamente")
...
statsFolder:AddButton("Reset Stats", function()
	...
	game:GetService("StarterGui"):SetCore("SendNotification", {
		Title = "Estatísticas",
		Text = "O progresso de estatísticas foram reinciados!",
		Duration = 0
	})
end)

statsFolder:AddButton("Copiar Estatísticas", function()
	local player = game.Players.LocalPlayer
	local statsText = "Estatísticas de Muscle Legends:\n\n"
	...
	if hasStartedTracking then
		...
		statsText = statsText .. "--- Estatísticas de sessão ---\n"
...
local petDropdown = pets:AddDropdown("Selecionar Pet", function(text)
	selectedPet = text
	print("Pet selecionado: " .. text)
end)
...
misc1Folder:AddButton("Remove Portals", function()
	...
	game:GetService("StarterGui"):SetCore("SendNotification", {
		Title = "Destruir Portal",
		Text = "Portal destruido",
		Duration = 0
	})
end)
...
misc3Folder:AddSwitch("No-Clip", function(bool)
	...
	if bool then
		...
		game:GetService("StarterGui"):SetCore("SendNotification", {
			Title = "No-Clip Ativado",
			Text = "Agora você pode atravessar os objetos",
			Duration = 0
		})
	end
end)
...
misc3Folder:AddSwitch("Jumpy Infinite", function(bool)
	...
	if bool then
		...
		game:GetService("StarterGui"):SetCore("SendNotification", {
			...
			Text = "Agora pode pular infinitamente",
			Duration = 0
		})
	end
end)
...
local timeDropdown = misc3Folder:AddDropdown("Change Time", function(selection)
	...
	game:GetService("StarterGui"):SetCore("SendNotification", {
		Title = "Hora trocada",
		Text = "A hora do dia foi trocada para: " .. selection,
		Duration = 0
	})
end)
...
rebirthFarmFolder:AddLabel("OP Rebirth Farm (From Zeus)")
	...
	task.spawn(function()
		while isRunning do
			...
			while isRunning and player.leaderstats.Strength.Value < rebirthCost do
				-- Rep mais rápido, 20 rep calls com intervalo mínimo
				for i = 1, 20 do
...
local function updateTargetPlayerLabel()
	if _G.targetPlayer == "" then
		targetPlayerLabel.Text = "Jogador Marcado: Nenhum"
	else
		targetPlayerLabel.Text = "Jogador Marcado: " .. _G.targetPlayer
	end
end
...
teleportTab:AddButton("Secret Area", function()
	...
	game:GetService("StarterGui"):SetCore("SendNotification", {
		Title = "Teleporte",
		Text = "Teleportando a Área Secreta",
		Duration = 0
	})
end)
...
teleportTab:AddButton("Tiny Island", function()
	...
	game:GetService("StarterGui"):SetCore("SendNotification", {
		Title = "Teleporte",
		Text = "Teleportando a Área Pequena",
		Duration = 0
	})
end)
...
teleportTab:AddButton("Teleport Frozen", function()
	...
	game:GetService("StarterGui"):SetCore("SendNotification", {
		Title = "Teleporte",
		Text = "Teleportando a Área Congelada",
		Duration = 0
	})
end)
...
teleportTab:AddButton("Mythical", function()
	...
	game:GetService("StarterGui"):SetCore("SendNotification", {
		Title = "Teleporte",
		Text = "Teleportando a Área Mística",
		Duration = 0
	})
end)
...
teleportTab:AddButton("Inferno", function()
	...
	game:GetService("StarterGui"):SetCore("SendNotification", {
		Title = "Teleporte",
		Text = "Teleportando a area Inferno",
		Duration = 0
	})
end)
...
teleportTab:AddButton("Legend", function()
	...
	game:GetService("StarterGui"):SetCore("SendNotification", {
		Title = "Teleporte",
		Text = "Teleportando a area das Lendas",
		Duration = 0
	})
end)
...
teleportTab:AddButton("Muscle King Gym", function()
	...
	game:GetService("StarterGui"):SetCore("SendNotification", {
		Title = "Teleporte",
		Text = "Teleportando ao Rei do Musculo",
		Duration = 0
	})
end)
...
teleportTab:AddButton("Brawl Lava", function()
	...
	game:GetService("StarterGui"):SetCore("SendNotification", {
		Title = "Teleporte",
		Text = "Teleportando a ilha de Lava",
		Duration = 0
	})
end)
...
teleportTab:AddButton("Brawl Desert", function()
	...
	game:GetService("StarterGui"):SetCore("SendNotification", {
		Title = "Teleporte",
		Text = "Teleportando ao ilha de Deserto",
		Duration = 0
	})
end)
...
teleportTab:AddButton("Brawl Regular", function()
	...
	game:GetService("StarterGui"):SetCore("SendNotification", {
		Title = "Teleporte",
		Text = "Teleportando a Combate de Praia",
		Duration = 0
	})
end)
...
noteTab:AddLabel("Legends Script")
...
noteTab:AddLabel("criado por: ZeusLGD")
noteTab:AddLabel("Créditos para Legends Store")
